...
//      System.out.println("url is " + url.toString());
      ...
      if (urlPath.endsWith(".jar")) {
        ...
        try {
          if (urlPath.contains(".m2")) continue; // Maven repositroy occasionally has a target directory in it, its an invalid path, so ignore it

          checkPath(urlPath, notifier,
                  "/target/classes/", "src/main/webapp",
                  "src/main/resources", "src/main/resources/META-INF/resources/"
            );
          ...
          checkPath(urlPath, notifier, testClassPath, "src/test/webapp", "src/test/resources");
  ...
  public synchronized static void loadIntoSystemProperties(String appName, String appPath) {
    ...
    if (appPath != null) {
      ...
      if (parent.isFile())
	    ...
	    // if app name is fat-domain, 
	    // parent (the appPath) will be .../fat-domain/src/test/resources
        // and this will attempt to load  .../fat-domain/src/test/resources/war.properties
